...
IF(wxWidgets_FIND_STYLE STREQUAL "win32")
    ...
    MACRO(WX_FIND_LIBS p_unv p_ucd p_dbg)
        ...
        FOREACH(LIB png tiff jpeg zlib regex expat)
            ...
            FIND_LIBRARY(WX_${LIB}${p_dbg}
                NAMES
                wx${LIB}${p_ucd}${p_dbg} # for regex
                wx${LIB}${p_dbg}
                PATHS ${s_wxLibDir}
                NO_DEFAULT_PATH
         )
            GET_FILENAME_COMPONENT(WX_${LIB}${p_dbg} ${WX_${LIB}${p_dbg}} NAME)
        ...
        FIND_LIBRARY(WX_base${p_dbg}
            NAMES
            wxbase${wxWidgets_PFVERSION}${p_ucd}${p_dbg}
            PATHS ${s_wxLibDir}
            NO_DEFAULT_PATH
     )
        GET_FILENAME_COMPONENT(WX_base${p_dbg} ${WX_base${p_dbg}} NAME)
        ...
        FOREACH(LIB net odbc xml)
            ...
            FIND_LIBRARY(WX_${LIB}${p_dbg}
                NAMES
                wxbase${wxWidgets_PFVERSION}${p_ucd}${p_dbg}_${LIB}
                PATHS ${s_wxLibDir}
                NO_DEFAULT_PATH
         )
            GET_FILENAME_COMPONENT(WX_${LIB}${p_dbg} ${WX_${LIB}${p_dbg}} NAME)
        ...
        FIND_LIBRARY(WX_mono${p_dbg}
            NAMES
            wxmsw${p_unv}${wxWidgets_PFVERSION}${p_ucd}${p_dbg}
            PATHS ${s_wxLibDir}
            NO_DEFAULT_PATH
     )
        GET_FILENAME_COMPONENT(WX_mono${p_dbg} ${WX_mono${p_dbg}} NAME)
        MARK_AS_ADVANCED(WX_mono${p_dbg})
        
        # Find wxWidgets multilib libraries
        FOREACH(LIB core adv aui html media xrc dbgrid gl qa stc richtext aui)
            SET(WX_${LIB}${p_dbg} "WX_${LIB}${p_dbg}-NOTFOUND" CACHE FILEPATH "Cleared." FORCE)
            FIND_LIBRARY(WX_${LIB}${p_dbg}
                NAMES
                wxmsw${p_unv}${wxWidgets_PFVERSION}${p_ucd}${p_dbg}_${LIB}
                PATHS ${s_wxLibDir}
                NO_DEFAULT_PATH
         )
            GET_FILENAME_COMPONENT(WX_${LIB}${p_dbg} ${WX_${LIB}${p_dbg}} NAME)
    ...
    #
    MACRO(WX_SET_LIBRARIES _LIBS p_dbg)
        ...
        ELSE(WX_USE_REL_AND_DBG)
            FOREACH(LIB ${${_LIBS}})
                ...
                ELSE(WX_${LIB}${p_dbg})
                    DBG_MSG("- not found due to missing WX_${LIB}${p_dbg}=${WX_${LIB}${p_dbg}}")
                    SET(wxWidgets_FOUND FALSE)
                ENDIF(WX_${LIB}${p_dbg})
            ENDFOREACH(LIB)
    ...
    IF(s_wxRootDir)
        ...
        IF(wxWidgets_USE_SHARED)
            FIND_PATH(wxWidgets_LIB_DIR
                ...
                PATHS
                ${s_wxRootDir}/lib/${s_wxCompiler}_dll   # prefer shared
        ...
        ELSE(wxWidgets_USE_SHARED)
            FIND_PATH(wxWidgets_LIB_DIR
                NAMES wxpng.lib wxpngd.lib
                PATHS
                ${s_wxRootDir}/lib/${s_wxCompiler}_lib   # prefer static
        ...
        IF(s_wxLibDir)
            ...
            #find version number from the wxbaseNN.lib file
            FILE(GLOB VERSIONDETECT  ${s_wxLibDir}/wxbase*.lib)
            STRING(REGEX REPLACE ".*wxbase([0-9][0-9]).*" "\\1" wxWidgets_PFVERSION "${VERSIONDETECT}")
            DBG_MSG("wxWidget install path: ${s_wxRootDir} version:  ${VERSIONDETECT} version:  ${wxWidgets_PFVERSION}")
            ...
            IF(s_wxConfiguration)
                ...
                IF(NOT WX_${wxWidgets_CONFIGURATION}_FOUND)
                    MESSAGE(FATAL_ERROR "${wxWidgets_CONFIGURATION} not found")    
                ...
                IF (EXISTS ${s_wxRootDir}/include/wx/wx.h)
                    SET(wxWidgets_INCLUDE_DIRS ${s_wxRootDir}/include)
                ELSE (EXISTS ${s_wxRootDir}/include/wx/wx.h)
                    DBG_MSG("WXWIDGET_FOUND FALSE because s_wxRootDir=${s_wxRootDir} has no ${s_wxRootDir}/include/wx/wx.h")
                    SET(wxWidgets_FOUND FALSE)
                ENDIF (EXISTS ${s_wxRootDir}/include/wx/wx.h)
                
                # Set wxWidgets lib setup include directory.
                IF (EXISTS ${s_wxLibDir}/${wxWidgets_CONFIGURATION}/wx/setup.h)
                    SET(wxWidgets_INCLUDE_DIRS ${wxWidgets_INCLUDE_DIRS}
                    ${s_wxLibDir}/${wxWidgets_CONFIGURATION})
                ELSE (EXISTS ${s_wxLibDir}/${wxWidgets_CONFIGURATION}/wx/setup.h)
                    DBG_MSG("WXWIDGET_FOUND FALSE because  ${s_wxLibDir}/${wxWidgets_CONFIGURATION}/wx/setup.h does not exists.")
                    SET(wxWidgets_FOUND FALSE)
                ...
                IF (EXISTS ${s_wxRootDir}/contrib/include)
                    SET(wxWidgets_INCLUDE_DIRS ${wxWidgets_INCLUDE_DIRS} ${s_wxRootDir}/contrib/include)
                ENDIF (EXISTS ${s_wxRootDir}/contrib/include)
                ...
                WX_SET_LIBRARIES(wxWidgets_USE_LIBS "${wxWidgets_DBG}")
