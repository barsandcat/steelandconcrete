...
namespace QuickGUI
{
	...
	void PropertyGrid::addChild(Widget* w)
	{
		...
		pgs->setWidth(getItemHeight() + mLabelWidth + getPropertyFieldWidth());
		...
		updateLabelWidth(pgs->getTextWidth() + pgs->mLabelBuffer);
	...
	void PropertyGrid::updateLabelWidth(float width)
	{
