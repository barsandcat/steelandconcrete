...
IconManager::IconManager()
{
    ...
    mIconMap[FONT] = wxBitmap(application_x_font_pcf);
    mIconMap[MESH] = wxBitmap(application_x_kpovmodeler);
    mIconMap[FILE_SYSTEM] = wxBitmap(folder);
    mIconMap[TEXTURE] = wxBitmap(image_x_generic);
    mIconMap[ZIP] = wxBitmap(utilities_file_archiver__);
    mIconMap[UNKNOWN] = wxBitmap(unknown);
}
