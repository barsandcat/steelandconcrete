...
boolean win_socket::open(int listen_queue_size, int sock_type, int flags)
{
    unsigned short port = 0;

    if (address != NULL) { 
	char* p;
	if ((p = strchr(address, ':')) == NULL || sscanf(p+1, "%hd", &port) != 1) 
	{
	    errcode = bad_address;
	    return False;
	}
...
socket_t* socket_t::create_dgram(char const* address, int flags)
{
    win_socket* sock = new win_socket(address);
    sock->open(0, SOCK_DGRAM, flags); 
    return sock;
}

int socket_t::get_ip(char const* address)
{
    int ip = inet_addr(address);
    if (ip < 0) { 
	struct hostent* hp;  // entry in hosts table
	if ((hp=gethostbyname(address)) == NULL || hp->h_addrtype != AF_INET)
	{
	    return -1;
	}
	ip = *(int*)hp->h_addr;
...
boolean win_socket::sendto(int ip_addr, int ip_port, void const* buf, size_t size)
{ 
    if (state != ss_open) { 
	errcode = not_opened;
	return False;
    ...
    int rc = ::sendto(s, (char*)buf, size, 0, (sockaddr*)&sin, sizeof sin);
    if (rc != (int)size) { 
	if (rc < 0) { 
	    errcode = errno;
	} else { 
	    errcode = message_truncated;
	}
	return false;
    }
    errcode = ok;
...
int win_socket::recvfrom(int& ip_addr, int& ip_port, void* buf, size_t size)
{
    if (state != ss_open) { 
	errcode = not_opened;
	return False;
    ...
    int rc = ::recvfrom(s, (char*)buf, size, 0, (sockaddr*)&sin, &sockaddr_len);
    if (rc < 0) { 
	errcode = errno;
    } else { 
	ip_addr = ntohl(sin.sin_addr.s_addr);
	ip_port = ntohs(sin.sin_port);
	errcode = ok;
    }
    return rc;
}
...
{ 
    address = addr != NULL ? strdup(addr) : NULL; 
    errcode = ok;
    s = INVALID_SOCKET;
}
...
{
    win_socket* sock = new win_socket(address);
    sock->open(listen_queue_size, SOCK_STREAM); 
    return sock;
}
...
void local_win_socket::get_error_text(char* buf, size_t buf_size)
{
	...
	break;
      case invalid_access_mode:
        strncpy(buf, "invalid access mode", buf_size);
	break;
      default: 	
...
boolean local_win_socket::sendto(int, int, void const*, size_t)
{
    Error = invalid_access_mode;
    return false;
}

int local_win_socket::recvfrom(int& ip_addr, int& ip_port, void* buf, size_t size)
{
    Error = invalid_access_mode;
    return -1;    
}
