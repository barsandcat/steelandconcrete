...
const char* unix_socket::unix_socket_dir = "/tmp/";
...
boolean unix_socket::open(int listen_queue_size, int sock_type, int flags)
{
    union
    {
        sockaddr    sock;
    ...
    if ((fd = socket(u.sock.sa_family, sock_type, 0)) < 0)
    {
        errcode = errno;
        return False;
    }
...
#endif
        while ((rc = ::read(fd, (char*)buf + size, max_size - size)) < 0 && errno == EINTR);
        if (rc < 0)
        {
            errcode = errno;
            return -1;
...
boolean unix_socket::sendto(int ip_addr, int ip_port, void const* buf, size_t size)
{
    if (state != ss_open)
    {
        errcode = not_opened;
    ...
    int rc = ::sendto(fd, (char*)buf, size, 0, (sockaddr*) & sin, sizeof sin);
    if (rc != (int)size)
    {
        if (rc < 0)
        {
...
int unix_socket::recvfrom(int& ip_addr, int& ip_port, void* buf, size_t size)
{
    if (state != ss_open)
    {
        errcode = not_opened;
...
#endif
    int rc = ::recvfrom(fd, (char*)buf, size, 0, (sockaddr*) & sin, &sockaddr_len);
    if (rc < 0)
    {
        errcode = errno;
    }
